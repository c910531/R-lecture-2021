...
## ggplot2 라이브러리
# 1. geom_histogram (position='stack','dodge')
gapminder %>% 
    filter(year==2007) %>% 
...
# 5. scale_fill_brewer() # 색 지정 팔레트 설정
...
# spectral 팔레트 (palette='Blues' or 'Oranges' ... 변경 가능)
gapminder %>% 
    filter(lifeExp>70) %>% 
    group_by(continent) %>% 
    summarise(n=n_distinct(country)) %>% 
...
# 그래프 표시 순서 (reorder은 continent를 n을 기준으로 오름차 순 정렬)
gapminder %>% 
    filter(lifeExp>70) %>% 
    group_by(continent) %>% 
    summarise(n=n_distinct(country)) %>% 
