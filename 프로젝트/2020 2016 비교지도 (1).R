...
map <- geojson_read('c:/workspace/R/data/map/daejon.json', what='sp')
leaflet(map) %>% 
...
cities <- read.csv('data/projectdata/t_total.csv')
...
diff1620 <- data.frame(name = c('대덕구', '동구', '서구', '유성구', '중구'), 
                        거래금액=avg20$t_ave20/avg16$t_ave16)
diff1620

map@data <- merge(map@data, diff1620, by='name', sort = F)
...
leaflet(map) %>% 
    ...
    addProviderTiles('Stamen.TonerLite') %>% 
    addPolygons(
        fillColor = ~pal(as.numeric(diff1620$거래금액)),
        weight = 2,
        opacity = 1,
        color = 'white',
        dashArray = '3',
